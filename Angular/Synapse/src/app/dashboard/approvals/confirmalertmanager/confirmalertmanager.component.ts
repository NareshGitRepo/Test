...
@Component({
  selector: 'app-confirmalertmanager',
  templateUrl: './confirmalertmanager.component.html',
  styleUrls: ['./confirmalertmanager.component.scss']
})
export class ConfirmalertmanagerComponent implements OnInit {
  OnlineAlertForm: FormGroup;
  ApprovalsList: ICheckerData;
  loading: boolean = false;
  approveData: ApproveData;
  constructor(private fb: FormBuilder, private dialog: MatDialog, public dialogRef: MatDialogRef<ConfirmalertmanagerComponent>,
  ...
  ngOnInit() {
    this.OnlineAlertForm = this.fb.group({
  ...
  approveOnlineAlerts() {
    const dialogRef = this.dialog.open(ConfirmApprovalComponent, {
      width: '500px',
      data: this.translate.instant('ApprovalModule.confirmMessageApproval') + this.ApprovalsList.onlineAlertName
    });
  ...
  rejectOnlineAlerts() {
    const dialogRef = this.dialog.open(ConfirmApprovalComponent, {
      width: '500px',
      data: this.translate.instant('ApprovalModule.confirmMessageReject') + this.ApprovalsList.onlineAlertName
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.loading=true;
        this.approveData = {
          checkId: this._checkerData.checkId,
          reason: this.OnlineAlertForm.value.rejectReason,
          status: 0
        }
