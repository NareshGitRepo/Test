...
import { ICreateRoom, IRoom } from '../_model/serviceModel';
...
export class RoommappingComponent implements OnInit {
  ...
  getRoomsByFloorId() {
    ...
    this.loading = true;
    this._mapRoomsService.getRoomsByFloorId(this.data.floorId).subscribe((response: IRoom[]) => {
      if (response.length > 0) {
  ...
  roomTypeSelect(roomTypes, action?: boolean) {
    ...
    if (roomTypes.length > 0) {
      roomTypes.forEach(element => {
        ...
        let roomListIndex = this.checkedForms.controls.findIndex(x => x.value.rooms[0].roomType == element);
        console.log('roomListIndex :: ', roomListIndex);
        if (roomListIndex == -1) {
          let roomdata = [];
          this.roomsData.forEach(room => {
            if (this.assignedRooms.findIndex(data => data.roomNumber == room.roomNumber) != -1)
              roomdata.push(room.roomId + '');
          });
          console.log('roomdata=>', roomdata);
          this.createData(this.roomsData, roomdata);
        }

      });
      this.checkedForms.controls.forEach(data => {

        let indx = roomTypes.findIndex(x => x == data.value.rooms[0].roomType);
        console.log('indx :: ', indx);

        if (indx == -1) {
          indx = this.checkedForms.controls.findIndex(x => x.value.rooms[0].roomType == data.value.rooms[0].roomType);
  ...
  onSubmit() {
    ...
    this.roomMappingForm.value.roomNoList.forEach(element => {
      ...
      let roomList = {
        roomType: element.rooms[0].roomType,
