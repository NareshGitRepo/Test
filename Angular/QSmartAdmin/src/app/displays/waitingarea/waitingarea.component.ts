...
export class WaitingareaComponent implements OnInit, OnDestroy {
  ...
  constructor(private route: ActivatedRoute, private router: Router, public dialog: MatDialog, private displayService: DisplayService, private appConfig: AppConfig) {
    ...
    this.audio.load();
    this.tokenThemes = this.appConfig.gettokenThemes();   console.log('this.tokenThemes',this.tokenThemes)
    this.displayLanglength = Object.keys(this.tokenThemes).length;     
    console.log("displayLanglength=>",this.displayLanglength);

    this.displayLang = this.appConfig.getdisplayLang() != null ? this.appConfig.getdisplayLang() : 0;     console.log('this.displayLang',this.displayLang)
    ...
    let dval1 = this.appConfig.getReceptionInterval();                    console.log('dval1',dval1)
    this.slidevalue = dval1 != null ? (dval1 < 2 ? 2 : dval1) : 2;         console.log('this.slidevalue',this.slidevalue)
   
    dval1 = this.appConfig.gettokenPopInterval();                console.log('dval1',dval1)
    this.tokenPopInterval = dval1 != null ? (dval1 < 5 ? 5 : dval1) : 5;                   console.log('this.tokenPopInterval',this.tokenPopInterval)
    ...
    if (this.displayid && this.displayid != '') {
      ...
      if (this.WaitingListParts.length > 1) {
        ...
        if (this.subscription) {
          ...
          this.WaitingListFinal = this.WaitingListParts[0];           console.log('WaitingListFinal', this.WaitingListFinal)
          this.subscription = Observable.interval(this.slidevalue * 1000).subscribe(data => {
            ...
            console.log('WaitingListFinal', this.WaitingListFinal)
            i++;
          });
        }
      }
      ...
      if (this.displayLang == 0)
        ...
        console.log(' this.subscriptionLang', this.subscriptionLang)
      }
      else
        this.arbicFlag = false;
    }
  ...
  repeatAlert(dataT: any, i: number) {
    ...
    if (dataT.length > i && this.alertloading) {
      if (dataT[i].roomNumber != null && dataT[i].roomNumber != '') {
        ...
        const dialogRef = this.dialog.open(TokenalertComponent, this.getDialogConfig({ 
          data: dataT[i], 
          colourdata: (this.tokenThemes[dataT[i].serviceColor] == null ? this.tokenThemes['0'] : this.tokenThemes[dataT[i].serviceColor]), 
          langData: this.displayLang }) 
          );
        this.playAudio();
