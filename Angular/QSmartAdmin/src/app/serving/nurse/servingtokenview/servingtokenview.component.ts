...
export class ServingtokenviewComponent implements OnInit {
  ...
  constructor(private dialogRef: MatDialogRef<ServingtokenviewComponent>, @Inject(MAT_DIALOG_DATA) private serviceInfo: IServingTokenNData,
    ...
    if (this.serviceInfo.deptname == '') {
      ...
      console.log("serviceArray=>", this.serviceArray, 'serviceInfo',this.serviceInfo);

      this.dataSource = new MatTableDataSource<IServingTokenN>(this.serviceArray);     console.log('dataSource',this.dataSource)
    }
    else {
      ...
      if (result) {
        ...
        this._service.servingEndTokenOtherResource(row.ticketId).subscribe((response: IResponse) => {
          if (response.status) {
            ...
            if (index != -1) {
              this.serviceArray.splice(index, 1);     console.log('this.serviceArray.splice(index, 1)',this.serviceArray.splice(index, 1))
              ...
              if (index1 != -1) {
                ...
                console.log('this.serviceInfo.ServingTokenNData.splice(index1, 1)',this.serviceInfo.ServingTokenNData.splice(index1, 1))
              }
              this.dataSource = new MatTableDataSource<IServingTokenN>(this.serviceArray);
            }
