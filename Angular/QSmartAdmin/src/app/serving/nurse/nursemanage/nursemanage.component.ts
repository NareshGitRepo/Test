...
import { IDeptList, IDoctorAndService, IDoctorInfo, IDoctro, IRequestForNext, IResponse, IResponseForNext, IRoom, IService, IServingTokenN, IServingTokenNData, IServingTokens, IToken } from '../../_model/tokenmodel';
...
export class NursemanageComponent implements OnInit, OnDestroy {
  ...
  getServingTokensOtherResource(action: boolean) {
    ...
    this.tokenService.getServingTokensOtherResource().subscribe((response: IServingTokenN[]) => {
      console.log('servingTokenOther',response)
      this.servingTokenData = [];
      if (response && response != null) {
        let servingTokenData = _.groupBy(response, 'deptName');      console.log('servingTokenData',servingTokenData)
        ...
        console.log("IServingTokenN=>",this.servingTokenData);
      }
      // else {
      //   this.doctorList = [];
      //   this.ServiceList = [];
