...
export class AppComponent {
  ...
  empdata:EmpList[]=[
    ...
    {empid:1087, empname:'sony',position:'clerk',salary:5000},
  ...
  onSelect(value){
      ...
      if(value==1){
       filterData = this.empdata.filter(obj=>{ obj.salary<5000 });
       console.log('emplist=>',this.empdata)
       console.log('below 5000 =>',filterData)
      }
      else if(value==2){
        filterData = this.empdata.filter(obj=>{ obj.salary>5000 && obj.salary<10000});
        console.log('5000 to 10000 =>',filterData)
      }
      else 
      {
        filterData = this.empdata.filter(obj =>{ obj.salary>10000});
