...
import { IRegistration, IDateInfo } from '../_model/registration.model';
...
export class SuccessmanageComponent implements OnInit {
  ...
  constructor(private registration: RegistrationService, private alertMessage: AlertMessageService, private appconfig: AppConfig,
    ...
    if (this._tokenInfo && tokenData) {
      ...
      this.registration.getCurrentDate().subscribe((response:IDateInfo)=>{
  ...
  getReport() {
    this.successRegistrationForm.get('searchValue').setValue(null);
  ...
  ExportTOExcel() {
    ...
    const ws: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.totalregistrationList.map(data=> _.pick(data, this.displayedColumns)));
    ...
    let tdy = new Date();
      ...
      /* save to file */
      XLSX.writeFile(wb, 'SuccessRegistration_Report_' + dateDownload + '.xlsx');
  }
  ExportTOCsv() {
    console.log("this.menuInfo", this.totalregistrationList[0]);
    console.log("this.menuInfo", Object.keys(this.totalregistrationList[0]));
    var options = {
      noDownload: false,
    ...
    new Angular5Csv(this.reportsList.map(data=> _.pick(data, this.displayedColumns)), 'SuccessRegistration_Report_' + dateDownload,options);   
  }
