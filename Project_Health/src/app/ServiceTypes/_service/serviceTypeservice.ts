...
export class ServiceTypeService {
  ...
  deleteService(srTypeId: number,floorId:number) {
    let _apiurlsdetials = this.apiUrls.getApiServiceUrlByComponentAndMethod(this.component, "deleteService")
    console.log("URL Get from config : =====>  ", _apiurlsdetials);
    if (_apiurlsdetials) {
        console.log("URL Get from config : ---->  ", _apiurlsdetials.url);
        let _apiUrl = _apiurlsdetials.url.replace("{serviceTypeId}", ''+srTypeId).replace("{floorId}",''+floorId);
