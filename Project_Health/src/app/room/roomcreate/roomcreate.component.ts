  ...
  onSubmit() {
    ...
    if (this.roomForm.value) {
      ...
      {  let roomData = {
        ...
        this.invalidRoomsArr.push((data.roomNameEn  as string).trim());
      }
      });
     // console.log('this.invalidRoomsArr.length && this.createRoomData.rooms.length',this.invalidRoomsArr.length,this.createRoomData.rooms.length);
      
      if(this.invalidRoomsArr.length==0 && this.createRoomData.rooms.length>0) {
        this.loading = true;
      ...
      this.showAlert(this.translate.instant('roomModule.roomcreate.invalidRoomRange')+this.invalidRoomsArr, ActionType.FAILED);
    }
    }
  }
