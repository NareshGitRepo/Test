...
getVitalRoomsInfoByNurse(tokenInfo:docnurse): Observable<IroomAndtoken> {
    let _apiurlsdetials = this.apiUrls.getApiServiceUrlByComponentAndMethod(this.component, "getVitalRoomsInfoByNurse")
    console.log("URL Get from config : =====>  ", _apiurlsdetials);
    if (_apiurlsdetials) {
        console.log("URL Get from config : ---->  ", _apiurlsdetials.url);
        let _apiUrl = _apiurlsdetials.url;
        return this.consumer.serviceConsumer<IroomAndtoken>(_apiUrl, _apiurlsdetials.type,tokenInfo, '');
    }
    else {
        console.log("URL Get from config : ---->  ", "Url not found..");
        return Observable.throw({ error: { messages: "url not found" } });
