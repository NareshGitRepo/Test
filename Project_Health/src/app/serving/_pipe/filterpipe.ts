...
import { Iroom, DoctorRoom, Itoken } from "../_model/tokenmodel";
...
@Pipe({
  ...
  export class IgetRoomFilter implements PipeTransform {
    transform(item:Iroom[], totalData: DoctorRoom[], filter: Itoken): Iroom[] {
     
    let index =totalData.findIndex(x=>x.doctorId==filter.drId);
    console.log("getRoomFilter=>",totalData,filter,index);
    
    if(index!=-1)
    item= totalData[index].rooms;
    else
    item=[];
