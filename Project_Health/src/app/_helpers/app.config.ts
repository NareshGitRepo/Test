...
export class AppConfig {
  ...
  private getSettings(): Settings {
   ...
   UserMinValidator(len: number): AsyncValidatorFn {
      return (control: AbstractControl): Promise<ValidationErrors | null> | Observable<ValidationErrors | null> => {
      if (control.value != null && control.validator({} as AbstractControl).required) {
        const observable = Observable.create(function subscribe(observer) {
        observer.next((control.value as string).trim());
        observer.complete();
        });
        return observable
        .map(response => {
        console.log('UserMinValidator=>', response, control, control.validator({} as AbstractControl))
        return response.length < len ? { minlength: true } : null
        })
      }
      else return null;
      };
  ...
  getMaxServeToken():number{
    return (this.settings.maxservetoken<=99)?this.settings.maxservetoken:99;
  }
