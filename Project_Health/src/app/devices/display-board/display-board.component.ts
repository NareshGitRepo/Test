...
export class DisplayBoardComponent implements OnInit {
  ...
  changeDepartment(departments, action?: boolean) {
    ...
    if (!action) {
      ...
      if(this.displaybordform.value.serviceCollection)
      {
        let serviceId:number[]=this.displaybordform.value.serviceCollection;
        serviceId=serviceId.filter(x=>this.serviceList.findIndex(y=>y.serviceId==x)!=-1);
        this.displaybordform.get('serviceCollection').setValue(serviceId.length > 0 ? serviceId : null);
      }
    }
    console.log("totalDepartements ::", this.totalDepartements1, departments, this.serviceList, this.displaybordform.value.serviceCollection);
  }
  ...
  ngOnInit() {
    this.displaybordform = this.fb.group({
      displayId: [''],
      displayName: ['', Validators.compose([Validators.required,Validators.minLength(4)]),this.appConfig.UserMinValidator(4)],
