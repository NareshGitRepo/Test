...
import { AppConfig, ITokenInfo, ILoginDtos } from '../../_helpers/app.config';
...
export class MenuComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    if (this._tokenInfo && tokenData) {
      this._manualAction = tokenData.tokenSub.isManualPage;
      try {
        if (this.appconfig.getMenuLoadItems() && this.appconfig.getMenuLoadStatus()) {
          ...
          this.activeTab = menuIndex != -1 ? this._menuitemsLoad[menuIndex].link : 'dashboard';
          ...
          menuService.getUserRoleMenuInfoWithId().subscribe((data: MainMenu[]) => {
            ...
            let manuAction = currenturl == '/manualtoken' ? this._manualAction : true;
            if ((menuIndex != -1 || currenturl == '/profile') && manuAction) {
              this.activeTab = menuIndex != -1 ? this._menuitemsLoad[menuIndex].link : 'dashboard';
              this.menuitems = this._menuitemsLoad;
              ...
              if (!this.appconfig.getMenuLoadStatus()) {
                ...
                let redata = targetRouteConfig.filter(data => (data as any).path == 'about' || (data as any).path == 'profile' || (this.menuitems.findIndex(s => s.link == (data as any).path || s.submenu.findIndex(y => y.link == (data as any).path) != -1 || s.submenu.findIndex(y => (y.link).substring(0, y.link.indexOf('/')) == (data as any).path) != -1) != -1));
                if (!this._manualAction)
                  redata = redata.filter(data => (data as any).path != 'manualtoken');
